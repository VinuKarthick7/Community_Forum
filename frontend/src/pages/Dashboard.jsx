...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        // Fetch all posts and filter by user (simple approach for MVP)
        api.get('/posts?limit=100')
            .then(({ data }) => {
                const mine = data.posts.filter((p) => p.author?._id === user._id || p.author === user._id);
                setPosts(mine);
            })
...
const styles = {
    ...
    avatarLg: { width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #a78bfa)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1.5rem', color: '#fff', flexShrink: 0 },
    name: { fontSize: '1.3rem', fontWeight: 800 },
    email: { color: 'var(--text-secondary)', fontSize: '0.88rem', marginTop: '0.1rem' },
    roleBadge: { display: 'inline-block', marginTop: '0.4rem', padding: '0.15rem 0.55rem', borderRadius: 999, fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', background: 'var(--accent-light)', color: 'var(--accent-hover)' },
    ...
    statNumber: { fontSize: '2rem', fontWeight: 800, color: 'var(--accent-hover)' },
